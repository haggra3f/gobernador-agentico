...
**Purpose:** To guarantee all user-facing communication is clear, consistent, and correctly attributed by centralizing the final rendering step.
...
Before returning ANY response to the user, the Orchestrator (or the main thread) MUST perform this final, non-negotiable sequence:
...
1.  **Identify Originator and Response**: After an agent (e.g., `desarrollador`) completes its task, capture its `raw_response` and its `originating_agent_name`.



2.  **Invoke the Announcer**: Pass both the `originating_agent_name` and the `raw_response` to the `presentation-agent`.



3.  **Display the Final Result**: The `presentation-agent` will return the complete, final message, including attribution. Display this message to the user without any modification.
...
1.  **Originating Agent:** `inventory-librarian` produces the raw text: `"[SUCCESS] Added 'Milk' to inventory"`.



2.  **Orchestrator** knows the originator is `inventory-librarian`.



3.  **Orchestrator** invokes `presentation-agent` with `originating_agent_name="Agente de Inventario"` and `raw_response="[SUCCESS]..."`.



4.  **`presentation-agent`** does its work and returns a single, complete string: `**Agente de Inventario:** He añadido 'Leche' a tu lista de artículos disponibles.`



5.  **User sees the final, complete, and correctly attributed response.**
...
-   **Guaranteed Attribution:** The responsibility is centralized in the presentation agent, ensuring it's never missed.



-   **Simplicity:** The Orchestrator's role is simplified, reducing the chance of errors.



-   **Robustness:** Creates a single source of truth for all user-facing communication.
